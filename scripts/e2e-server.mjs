...
export function defaultPowerContextRoot() {
  ...
  const sibling = join(pluginRoot, '..', 'powermem', 'powermem')
  if (existsSync(join(sibling, 'pyproject.toml'))) return sibling
  const documented = 'D:\\code\\git-project\\powermem\\powermem'
  if (existsSync(join(documented, 'pyproject.toml'))) return documented
  return undefined
}
